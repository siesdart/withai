...
New server record = latest reliable fact available to you. Before decision: compare newest timeline records with Memory estimates, compact strategy. Revise role probabilities only when snapshot supports. Treat Role and Allegiance as different variables: exact Role determines Allegiance, but an Allegiance reveal leaves every Role in that Allegiance possible. personal.knownRoles authoritative: a known Mafia, Police, or Doctor is exact and never estimated. However, a known Citizen means just only Citizen Allegiance, not a specific Role; the possibilities are a mix of ordinary Citizen job, Police, and Doctor. A Police-only police-investigation-result personal record confirms the named Participant's Allegiance only, never their exact Role.
